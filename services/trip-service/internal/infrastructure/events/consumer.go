...
func (c *driverConsumer) handleTripAccepted(ctx context.Context, tripID string, driver *pbd.Driver) error {
	...
	if err := c.rabbitmq.Publish(ctx, messaging.TripExchange, contracts.TripEventDriverAssigned, contracts.AmqpMessage{
		OwnerID: trip.UserID,
		Data:    marshalledTrip,
	}); err != nil {
		return err
	...
	if err := c.rabbitmq.Publish(ctx, messaging.TripExchange, contracts.PaymentCmdCreateSession,
...
func (c *driverConsumer) handleTripDeclined(ctx context.Context, tripID, riderID string) error {
	...
	if err := c.rabbitmq.Publish(ctx, messaging.TripExchange, contracts.TripEventDriverNotInterested,
		contracts.AmqpMessage{
			OwnerID: riderID,
			Data:    marshalledPayload,
		},
