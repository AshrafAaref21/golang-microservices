...
func (c *tripConsumer) handleFindAndNotifyDrivers(ctx context.Context, payload messaging.TripEventData) error {
	...
	if len(suitableIDs) == 0 {
		...
		if err := c.rabbitmq.Publish(ctx, messaging.TripExchange, contracts.TripEventNoDriversFound, contracts.AmqpMessage{
	...
	if err := c.rabbitmq.Publish(ctx, messaging.TripExchange, contracts.DriverCmdTripRequest, contracts.AmqpMessage{
